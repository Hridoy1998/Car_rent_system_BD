    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em] mt-1">Personal Mission & Fleet Deployment Center</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="lg:col-span-1 space-y-10">
                    <div class="bg-gray-900/40 backdrop-blur-3xl border border-white/5 p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden group h-full">
                        ...
                        <div class="text-center py-6">
                            ...
                            <h4 class="text-xl font-black text-white uppercase tracking-tighter mb-1">Combat Rating</h4>
                            <p class="text-[10px] text-gray-600 font-bold uppercase tracking-widest italic">Aggregated from {{ $reputation['review_count'] }} operational cycles</p>
                        </div>

                        <div class="mt-8 grid grid-cols-2 gap-4">
                            <div class="p-4 bg-white/5 border border-white/5 rounded-2xl transition-colors group-hover:bg-white/10">
                                <div class="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1 text-center">Identity Status</div>
                            ...
                            <div class="p-4 bg-white/5 border border-white/5 rounded-2xl transition-colors group-hover:bg-white/10">
                                <div class="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1 text-center">Total Yield</div>
                                <div class="text-sm font-black text-white text-center">{{ $stats['total_bookings'] }} Ops</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-gray-900/40 backdrop-blur-3xl border border-white/5 p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden h-full">
                         <div class="flex items-center justify-between mb-12">
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 font-bold uppercase tracking-widest mt-1">Monthly capital outflow trajectory</p>
                            </div>
                        </div>

                        <div class="h-48 flex items-end justify-between gap-4 px-4 relative">
                            @php $maxSpend = !empty($fiscalPulse) ? max($fiscalPulse) : 1; @endphp
                            ...
                            @if(empty($fiscalPulse))
                                <div class="absolute inset-0 flex flex-col items-center justify-center text-gray-800 text-[10px] font-black uppercase tracking-[0.4em] italic opacity-20">NO FINANCIAL SIGNAL RECEIVED</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col lg:flex-row gap-12 items-center">
                    <div class="w-full lg:w-1/3 aspect-[4/3] rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl relative rotate-[-2deg] group-hover:rotate-0 transition-transform duration-700">
                        ...
                        <div class="absolute bottom-6 left-6 flex items-center gap-2">
                             ...
                             <span class="text-[10px] font-black text-white uppercase tracking-widest">Ongoing Mission</span>
                        </div>
                    </div>
                    
                    <div class="flex-1 space-y-8">
                        <div>
                            <div class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-2">Live Deployment Tracked</div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                            <div class="space-y-1">
                                <div class="text-[8px] text-gray-600 font-black uppercase tracking-widest">Pickup Epoch</div>
                                <div class="text-[11px] text-white font-black uppercase">{{ \Carbon\Carbon::parse($activeTrip->start_date)->format('M d, Y') }}</div>
                            </div>
                            <div class="space-y-1">
                                <div class="text-[8px] text-gray-600 font-black uppercase tracking-widest">Return Epoch</div>
                            ...
                            <div class="space-y-1 text-right">
                                <div class="text-[8px] text-gray-600 font-black uppercase tracking-widest italic">Capital Settle</div>
                                <div class="text-[13px] text-emerald-400 font-black">৳{{ number_format($activeTrip->total_price) }}</div>
                            </div>
                        </div>
                        
                        <div class="flex flex-wrap gap-4 pt-4">
                            <a href="{{ route('customer.bookings.index') }}" class="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl transition-all shadow-[0_0_30px_rgba(99,102,241,0.3)]">Manage Mission Details</a>
                            <a href="{{ route('bookings.messages.index', $activeTrip) }}" class="px-8 py-4 bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-widest border border-white/10 rounded-2xl transition-all">Engage Operator Comms</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-8">
                <div class="flex items-center justify-between px-2">
                    ...
                    <a href="{{ route('home') }}" class="text-[10px] font-black text-gray-600 hover:text-white transition-colors uppercase tracking-[0.3em]">Full Marketplace →</a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    ...
                    <div class="group relative bg-gray-900/40 backdrop-blur-2xl border border-white/5 p-6 rounded-[2.5rem] shadow-2xl transition-all hover:scale-[1.02] hover:bg-gray-900/60 overflow-hidden">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-white/5">
                            ...
                            <a href="{{ route('cars.show', $fav->car) }}" class="text-[9px] font-black text-indigo-400 hover:text-indigo-300 transition-colors uppercase tracking-widest">Engage →</a>
                        </div>
                    </div>
                    @empty
                    <div class="lg:col-span-4 p-24 text-center bg-gray-900/20 border-2 border-dashed border-white/5 rounded-[4rem]">
                         ...
                         <h4 class="text-gray-500 font-black text-xs uppercase tracking-[0.4em] mb-2">Wishlist Vacancy Detected</h4>
                         <p class="text-gray-700 text-[10px] font-bold uppercase tracking-widest italic max-w-sm mx-auto leading-relaxed">Your fleet aspirations are currently dormant. Mark prime assets in the marketplace to monitor them here.</p>
                         <a href="{{ route('home') }}" class="inline-block mt-8 px-8 py-3 bg-white/5 hover:bg-white/10 text-white text-[9px] font-black uppercase tracking-widest border border-white/10 rounded-2xl transition-all">Explore Global Fleet</a>
                    </div>
                    @endforelse
                </div>
            </div>
