<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1">Global audit of vehicle
                    lease agreements and financial settlement</p>
            </div>
    ...
    <div class="py-12 bg-gray-950 min-h-screen text-slate-200">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div
                class="bg-gray-900/50 backdrop-blur-xl border border-white/10 p-6 rounded-[2rem] shadow-2xl flex flex-col md:flex-row gap-4 items-center justify-between">
                <x-search-bar :route="route('admin.bookings.index')" placeholder="Search by user, car, or ID..." />
                <div class="flex gap-2">
                    <select
                        class="bg-gray-950 border border-white/5 rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest text-gray-400 focus:ring-indigo-500">
            ...
            <div
                class="bg-gray-900/50 backdrop-blur-xl border border-white/10 overflow-hidden shadow-2xl rounded-[2.5rem]">
                                ...
                                <tr class="group hover:bg-white/[0.02] transition-colors cursor-pointer"
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div
                                                ...
                                                @if($booking->car->images->count() > 0)
                                                    <img src="{{ Storage::url($booking->car->images->first()->image_path) }}"
                                    ...
                                    <td class="py-8">
                                        <div class="space-y-4" onclick="event.stopPropagation()">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-6 h-6 rounded-md bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-[8px] font-black text-purple-400">
                                                    H</div>
                                                <a href="{{ route('profiles.show', $booking->car->owner) }}"
                                                    class="text-[10px] font-bold text-gray-300 hover:text-indigo-400 transition-colors">{{ $booking->car->owner->name }}</a>
                                            </div>
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-6 h-6 rounded-md bg-green-500/10 border border-green-500/20 flex items-center justify-center text-[8px] font-black text-green-400">
                                                    C</div>
                                                <a href="{{ route('profiles.show', $booking->customer) }}"
                                                    class="text-[10px] font-bold text-gray-300 hover:text-indigo-400 transition-colors">{{ $booking->customer->name }}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-8">
                                    ...
                                    <td class="py-8">
                                        ...
                                        @elseif($booking->status === 'approved')
                                            <span
                                                class="px-2 py-0.5 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-md text-[8px] font-black uppercase tracking-widest">Authorized</span>
                                        @elseif($booking->status === 'completed')
                                            <span
                                                class="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-md text-[8px] font-black uppercase tracking-widest">Succeeded</span>
                                        @else
                                            <span
                                                class="px-2 py-0.5 bg-white/5 text-gray-500 border border-white/10 rounded-md text-[8px] font-black uppercase tracking-widest">{{ $booking->status }}</span>
                                        @endif
                                        <div class="text-[8px] text-gray-700 font-bold uppercase mt-2">
                                        ...
                                        <div class="flex items-center justify-end gap-6">
                                            <div class="flex justify-end gap-2" onclick="event.stopPropagation()">
                                                <a href="{{ route('admin.bookings.show', $booking) }}"
                                                    class="p-3 bg-white/5 hover:bg-indigo-600 hover:text-white text-gray-400 rounded-2xl border border-white/5 transition-all shadow-lg">
                                                ...
                                                <form action="{{ route('admin.bookings.update', $booking) }}" method="POST">
                                                    ...
                                                    <button type="submit"
                                                        class="p-3 bg-red-600/10 hover:bg-red-600 hover:text-white text-red-500 rounded-2xl border border-red-500/20 transition-all shadow-lg"
                                                        title="Invoke Cancellation">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @if($bookings->isEmpty())
                <div class="py-24 text-center">
                    ...
                    <p class="text-gray-700 text-[10px] italic mt-1">Transaction ledger is currently devoid of matching
                        manifest entries.</p>
                </div>
            @endif
